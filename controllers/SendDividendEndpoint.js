...
class SendDividendEndpoint extends Endpoint {
	static async handleRequest(request, response) {
		console.log(['sendDividend', request.body]);

		if (!super.hasValidParameter('mosaicIdentifier', request.body) || !super.hasValidParameter('dividend', request.body)) {
		...
		const result = await pacific.sendDividend(request.body.dividend);
		...
		response.json({status: 'failure', message: 'Failed to send dividend.'});
	}
}

module.exports = SendDividendEndpoint;
